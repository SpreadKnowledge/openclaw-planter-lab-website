...
- タイトルは説明文や報告書の見出しよりも、個人ブログらしい自然な言い回しを優先する
- タイトルには読点を入れない
- 近ごろの好みは、読者に話しかけるようなタイトルや、軽い問いかけ型のタイトル
  - 例: `土の表面乾いてるけど大丈夫？`
- `進んでいました` のような報告調より、会話っぽくて引っかかりのある表現を選ぶ
- `かなり` `ちょっと○○でした` `少し` の連発のような、AIっぽく見えやすい強調やぼかしは避け、素直な言い方にする
- 豆知識記事は `播種後○日目` を付けない
- 栽培ログ系では `播種後○日目` を付けてもよいが、タイトル全体は固くしすぎない

## 公開時の位置情報と天気表現

- 栽培ブログでは、具体的な市区町村、駅名、地域名、沿岸部など、住んでいる場所を推測しやすい表現を出さない
- 天気予報を使う場合も、本文では `私の住んでいる近くの予報` `このあたりの予報` くらいにぼかす
- `南関東` のような広域表現でも、不要なら避ける
- CSVや元データのダウンロードリンクは、ユーザーが明示的に公開を希望した場合だけ出す
- 表や本文に、座標、観測地点、取得元の細かい地点名を含めない

## 投稿本文の見出し

現在の運用では、ブログ本文は見出しを並べた記録形式よりも、自然な本文だけで読める構成を優先します。

- 原則として `今日の観察` `今日やったこと` `次にやること` `OpenClawコメント` の各見出しは入れない
- 本文の流れだけで内容が伝わるように書く
- 必要な補足がある場合も、見出しで細かく区切りすぎず自然な段落でまとめる
- 記事末尾の署名 `（by クローラ）` は継続する
